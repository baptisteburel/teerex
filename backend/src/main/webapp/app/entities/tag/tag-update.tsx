...
export const TagUpdate = (props: RouteComponentProps<{ id: string }>) => {
  ...
  const tagNamesValues = Object.keys(TagNames);
  const handleClose = () => {
  ...
  return (
    <div>
      <Row className="justify-content-center">
        <Col md="8">
          <h2 id="ptiRexApp.tag.home.createOrEditLabel" data-cy="TagCreateUpdateHeading">
        ...
        <Col md="8">
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              <ValidatedField label="Tag Name" id="tag-tagName" name="tagName" data-cy="tagName" type="select">
                {tagNamesValues.map(tagNames => (
                  <option value={tagNames} key={tagNames}>
                    {tagNames}
                  </option>
                ))}
              </ValidatedField>
